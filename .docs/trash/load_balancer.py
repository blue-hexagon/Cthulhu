...
class ObjectTracker(metaclass=Singleton):
    """Tracks Clients by keeping a dictionary of hash:last_seen items.
...
class RoundRobinLoadBalancer(metaclass=Singleton):
    ...
    def __init__(self, queue_size, generator) -> None:  # noqa
        self.client_queue: Queue[Client] = Queue(maxsize=0)
    ...
    def get_next_client(self) -> Client:
        client: Client = self.client_queue.get()
        ...
        while True:
            if self.client_queue.empty():
                for i in range(random.randint(1, 5)):
                    self.add_client(clients[random.randint(0, len(clients) - 1)])
                sleep(random.uniform(0.1, 2.4))
                continue
...
if __name__ == "__main__":
    ...
    RoundRobinLoadBalancer(queue_size=4, generator=pass_gen).run()
