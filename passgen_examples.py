...
if __name__ == '__main__':
    def use_bruteforce():
        BruteforcePWFabricator(4, 4, filename="hex_upper__4_4.txt",
                               combinatoric_iterator=itertools.product).use_filewriter(
            [CharacterClass.DIGITS, CharacterClass.HEXDIGITS_UPPER]
        )
        bruteforce_map = BruteforcePWFabricator(1, 3, combinatoric_iterator=itertools.permutations).use_generator(
            ["abc"])
        while 1:
            try:
    ...
    def use_wordlist():
        WordlistPWFabricator(filename="wl_common__4_4.txt").use_filewriter(
            (["app", "app-"], ["script"], ["-ch2", "-ch1"]))
