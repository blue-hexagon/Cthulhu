...
def test_bruteforce_generator():
    bruteforce_pw_generator = BruteforcePWFabricator(2, 2, combinatoric_iterator=itertools.product).use_generator(
        ["0123456789abcdef"])
    while True:
        try:
...
def test_bruteforce_fabricator_permutation_fabricator():
    pw_permuter = BruteforcePWFabricator(2, 2, combinatoric_iterator=itertools.permutations).use_generator(
        ["0123456789abcdef"])
    while True:
        try:
