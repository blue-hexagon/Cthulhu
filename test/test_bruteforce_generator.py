...
def test_bruteforce_generator():
    pw_permuter = BruteforcePWGenerator(2, 2, combinatoric_iterator=itertools.product).use_generator(["0123456789abcdef"])
    while True:
        try:
...
def test_bruteforce_generator_permutations():
    pw_permuter = BruteforcePWGenerator(2, 2, combinatoric_iterator=itertools.permutations).use_generator(["0123456789abcdef"])
    while True:
        try:
